...
\begin{figure*}
	...
	\caption{Simulated population of the first qubit excited state ($\lg P_{\ket{10}}$ is shown with colour). Left picture is same as the right one but with transitions from the stationary Hamiltonian overlaid. The legend labels the transitions (notation as in the the main text) at the sweet spot current; if elsewhere any two lines cross, the labels should be swapped after the intersection.}
...
\begin{figure*}
	...
	\caption{Investigation of the power dependence of feature II. Dashed are the model curves turning to dotted when the model is no longer expected to be valid (see Section \ref{sec:theory}). Model values for $\Omega_2$ are the same for both panels and shown as annotations in (b). \textbf{(a)} Experiment (second cooldown; the parameters are different from \autoref{fig:experiment}). The power of the microwave source is increased from -20 to 0 dBm so that the amplitude grows linearly, and the proportional growth of the splitting is observed. \textbf{(b)} Simulation with an equal and increasing amplitudes of the driving $\Omega_{1,2}/2\pi=$4.5, 11.25, 22.5 and 45 MHz; other parameters as in \autoref{fig:two-tone}. Note that now colours show the population of the ground state ($\ket{00}$). The growth of the splitting is a clear indication of the light-dressing to be at the root of the effect.}
	\label{fig:zoom}
\end{figure*}
...
From \autoref{fig:statinary_levels} we conclude that avoided crossing in III is between two transitions: $\ket{00}\rightarrow\ket{12/3}$ and $\ket{10}\rightarrow \ket{02}$ which are of the same frequency when $\omega_1 = \omega_2-\alpha_2/2$. The latter process is discernible in \autoref{fig:statinary_levels} as it depopulates $\ket{10}$, but is not visible in \autoref{fig:two-tone}. From additional measurements and simulations, we have found that the size of the splitting depends on the driving power on the first transmon; the results are shown in \autoref{fig:zoom}~(a),(b). As one can see, the growth of the splitting with increasing power is linear: its width is roughly equal to the FWHM of the $\ket{10}$ spectral line which is approximately $\Omega_1/2\pi$ Hz. To fully quantify the size of the splitting, in Section \ref{sec:theory} we derive analytical expressions for the dashed curves fitting the spectral lines
...
Since we had already connected the additional avoided crossings with light dressing, it was natural to expect the Autler-Townes effect to cause them. For a three-level system, the standard A-T effect is reviewed in Appendix \ref{sec:3-level-at}. However, in our case the level structure is more complicated. 
...
Similarly, for feature I, the simultaneously excited transitions are $01 (10)$ and the two-photon $11/2$. However, as we will show, for the avoided crossing of feature I to be clearly observed it is required that $\Omega_{(1,2)} \gg \Omega_{(2,1)}$. Below, we give the detailed explanation concerning features II and III, and finally, I.

\paragraph{Features II and III.}  

In \autoref{fig:main_scheme}, we elaborate on how exactly the A-T effect would occur for feature III. For simplicity, we  \autoref{fig:main_scheme}~(a) demonstrates the unmodified level structure of the SAM. Since from numerical simulations we know that level $\ket{02}$ is not necessary to observe the splitting, it is only shown with a dashed line, and hereinafter we consider the first transmon as a two-level system.
...
\subsection{Shape of the extra avoided crossings}

In this section we will employ the model from \autoref{fig:main_scheme} to explain the experimentally observed splittings when only a single frequency excitation is applied. From here, we will use feature II as the example. In the laboratory frame, the two transitions forming the avoided crossing are $\left|01\right\rangle \rightarrow \left|20\right\rangle$ (one-photon) and $\left|00\right\rangle \rightarrow \left|21/3\right\rangle$ (three-photon). In the rotating frame with the second qubit at a small detuning $\Delta_2$ they both become two-photon since the second qubit splitting $\omega_2$ is replaced by $\Delta_2$ which is close to 0, and we are looking at transitions near $\omega_1+\alpha_1/2$. 

When $\Omega_2$ = 0 (no driving yet on the second qubit), the corresponding frequencies are:
\begin{equation}
\begin{aligned}
\omega_{(01-20)/2} &= (\alpha_1 + 2 \omega_{1} - \Delta_2)/2\\
 \omega_{21/2} &= (\alpha_1 + 2 \omega_{1} + \Delta_2)/2
\end{aligned}
\end{equation}
When the second qubit becomes dressed by $\Omega_2 \neq 0$, its splitting becomes $\Omega^R_2 =\sqrt{\Omega_{2}^2 + \left(\omega_{2} - \omega_{d}\right)^{2}}$, where $\omega_d$ is the driving frequency. Since we are simultaneously dressing the $\left|i0\right\rangle, \left|i1\right\rangle$ states and probing transitions between them, self-consistent equations have to be solved to find at which $\omega_d$ the sideband spectral lines appear:
\begin{equation}
\begin{aligned}
\omega_{(01-20)/2} &= \omega_d,\\
\omega_{21/2} &= \omega_d
\end{aligned}
\end{equation}

When $\Omega_2 = 0$, these equations yield indentical pairs of solutions due to the properties of the modulus $|\omega_2 - \omega_d|$. Intriguingly, they are the three-photon $\left|00\right\rangle \rightarrow \left|21/3\right\rangle$ and single-photon $\left|01\right\rangle \rightarrow \left|20\right\rangle$ transition frequencies in the laboratory frame:
\begin{equation}
\omega_d = \begin{cases} \frac{\alpha_1}{3} + \frac{2 \omega_{1}}{3} + \frac{\omega_{2}}{3}, \\ \alpha_1 + 2 \omega_{1} - \omega_{2}\end{cases}
\end{equation}
When $\Omega_2 \neq 0$, we obtain the following solutions:
\begin{equation}
\omega_d = \frac{2 \alpha_2}{3} + \frac{4 \omega_{1}}{3} - \frac{\omega_{2}}{3} \pm \frac{\sqrt{3 \Omega_{2}^{2} + \left( \alpha_1 + 2 (\omega_{1} - \omega_{2})\right)^{2}}}{3}
\label{eq:splitting_model}
\end{equation}
The minimal splitting between these lines is found at the resonant point where $\omega_2 = \omega_1 +  \alpha_1/2$ and equals $\frac{2}{\sqrt{3}} \Omega_2$. We find excellent agreement between our analytical model \eqref{eq:splitting_model} and both the experimental and simulated data as can be seen in \autoref{fig:zoom}. To plot the model curves we have fitted the $\omega_{(1,2)}(\Phi_e)$ transitions with simple linear functions and substituted them in \eqref{eq:splitting_model}; $\alpha_2$ is known from spectroscopy. T																																																																																																																																																																																								he only free parameter left was $\Omega_2$. We have repeated this approximation for a range of microwave powers to confirm the linear dependence of the splitting (see \autoref{fig:splitting_linear}).

\begin{figure}
	\includegraphics[width=\linewidth]{powerscan_1d}
	\caption{Linear dependence of the splitting size on the driving voltage from the microwave source $V_{rms} = \sqrt{10^{P_{exc}/10}\cdot 1 \text{ mW} \cdot 50\ \Omega}$.}
	\label{fig:splitting_linear}
\end{figure}

The upper branch is expected to deviate from the model (see dashed curves in \autoref{fig:zoom}) because of the avoided crossing marked as the secondary feature 3 which was not incorporated in the simple linear model. The small discrepancy between the experimental and simulated data in the upper branch is caused by a slight elevation of the lower sweet spot of the first transmon which has offset the avoided crossing 3 which becomes closer to the feature II; it is not important for our reasoning.
...
\begin{figure}
	...
	\caption{Simulated population of the ground state when the transmons are driven at different amplitudes $\Omega_{1,2}$. In the left (right), $\Omega_{1(2)}$ is increased while $\Omega_{2(1)} = \text{const}$. Two topologically different splittings may be observed}
	\label{fig:difdrive}
\end{figure}
