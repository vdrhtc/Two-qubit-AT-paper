...
\end{aligned}
\end{equation}
The first part is diagonal, and the remaining two will be treated as perturbations. To simplify futher calculations, we take the working point where $\Delta_1 = 0$ and $\Delta_2 = - \alpha_2/2$. In these conditions, $H^D$ becomes degenerate as can be seen in \eqref{eq:ham_matrix} of Appendix \ref{sec:H_mat}.
...
\begin{equation}
A^{(2)}_{1\rightarrow 6} = 2\pi\Omega_2^4 \frac{18 J^4 \left(2 \alpha + \text{$\Omega_1
		$}\right)^2}{\alpha ^{6}\Omega_1^2}
\end{equation}
\begin{equation}
A^{(2)}_{2\rightarrow 5} = 2\pi\Omega_2^4 \frac{18 J^4 \left(2 \alpha - \text{$\Omega_1
		$}\right)^2}{\alpha ^{6}\Omega_1^2}
\end{equation}
...
Interestingly, the self-consistent models for the observed effects are implying that multi-photon processes may change their order. For example, for features II and III, we observe a continuous transformation of a three-photon and a single-photon processes to the second order in \eqref{eq:two-photon}, and a two-photon to a zero-photon in \eqref{eq:zero-photon}.
...
\section{The dressed Hamiltonian in the matrix form} \label{sec:H_mat}

Since the Hamiltonian in the dressed basis is a useful illustration for \autoref{fig:main_scheme}, we provide it below in \eqref{eq:ham_matrix} for the case $\Delta_1 = 0$, $\delta = \omega_d^2 - 	\omega_d^1 = \omega_d^2 - \omega_1$.

Note that $\hat V_J = \hat S^T \hat H_{int} \hat S$ now has an sub- and super-diagonal block form coupling all the states $\ket{a, j}, \ket{a,j+1}$ and $\ket{b, j}, \ket{b,j+1}$.

The degeneracy between states $\ket{a, 0}, \ket{a,2}$ and $\ket{b, 0}, \ket{b,2}$ occurs only in the resonant case $2\Delta_2   + \alpha_2 = 0$ meaning that the second transmon frame is rotated at its two-photon transition frequency. In this situation, its single-photon subspace is detuned by $-\alpha/2$.
...
\begin{equation}
	\begin{split}
	\tilde{V}(t)=\exp(i H_0 t) \Omega\cos(\omega_d t)(\hat b+\hat b^{\dagger})\exp(-i H_0 t)=\\\frac{\Omega}{2} \exp(i H_0 t) \hat{B}(e^{-i\omega_d t}+e^{i\omega_d t})\exp(-i H_0 t)
	\end{split}
\end{equation}
Where, $\hat{b}^{\dagger}+\hat{b}=\hat{B}$.
Taking the matrix element between the initial and the final states $\ket{i}$ and $\ket{f}$, one immediately obtains
\begin{equation}
	\begin{split}
	\bra{f}U^{(1)}(t,t_0)\ket{i}=-i\int_{t_0}^{t}dt_1\tilde{V}(t_1)=\\-i\frac{\Omega}{2}\big[\int_{t_0}^{t}dt_1e^{i(\omega_f-\omega_i-\omega_d)t_1}\bra{f}\hat{B}\ket{i}+\\
	\int_{t_0}^{t}dt_1e^{i(\omega_f-\omega_i+\omega_d)t_1}\bra{f}\hat{B}\ket{i}\big]
	\end{split}
\end{equation}
...
\begin{equation}
	\begin{split}
	\bra{f}U^{(2)}(\infty)\ket{i}=\\\frac{\pi i\Omega^2}{2}\bigg[\delta(\omega_f-\omega_i+2\omega_d)\sum_j\frac{\bra{f}\hat{B}\ket{j}\bra{j}\hat{B}\ket{i}}{\omega_j-\omega_i+\omega_d}+\\
	\delta(\omega_f-\omega_i)\bigg(\sum_j\frac{\bra{f}\hat{B}\ket{j}\bra{j}\hat{B}\ket{i}}{\omega_j-\omega_i+\omega_d}+\\
	\sum_j\frac{\bra{f}\hat{B}\ket{j}\bra{j}\hat{B}\ket{i}}{\omega_j-\omega_i-\omega_d}\bigg)+\\
	\delta(\omega_f-\omega_i-2\omega_d)\sum_j\frac{\bra{f}\hat{B}\ket{j}\bra{j}\hat{B}\ket{i}}{\omega_j-\omega_i-\omega_d}\bigg]
	\end{split}
\end{equation}
...
\begin{split}
	W^{(2)}_{i\rightarrow f}=\frac{\pi\Omega^4}{8}\bigg|\sum_j\frac{\bra{f}\hat{B}\ket{j}\bra{j}\hat{B}\ket{i}}{\omega_j-\omega_i-\omega_d}\bigg|^2\delta(\omega_f-\omega_i-2\omega_d)=\\ A^{(2)}_{i\rightarrow f}\delta(\omega_f-\omega_i-2\omega_d)
\end{split}
\end{equation}
