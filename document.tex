...
These observation lead to understanding that the effect is caused by light dressing. In case III, the first transmon is dressed by a strong resonant field; in case II, the second one; finally, in case I, both transmons are dressed at the same time. We will discuss these effects in greater detail in \autoref{sec:theory}.
...
Since we had already connected the additional avoided crossings with light dressing, it was natural to expect the Autler-Townes effect to play a part. For a three-level system, the standard A-T effect is reviewed in Appendix \ref{sec:3-level-at}. However, in our case the situation is more complicated. We will first explain the feature III. Feature II is explained in the exactly same way. Feature I is of the same origin but is not discussed here in such detail. 

First of all, during the spectroscopy we apply only a single microwave tone, and thus is has to be simultaneously the coupler tone and the probe tone to observe the A-T effect. This configuration is possible due to the resonance condition between the transmons at III: the light is resonant with $\ket{10}$ and detuned from $\ket{01}$; however, it is instead resonant with a two-photon transition $\ket{02/2}$ (the resonance condition is $\omega_1 = \omega_2+\alpha_2/2$). Since the two-photon Rabi frequency is much smaller than the single-photon $\Omega_2$, it is natural to view this two-photon excitation as the probing process which does not affect the level structure. In other words, in our case the A-T coupling operator is $\hat H_{dr}^1$ (strong), and the probing operator is $\hat H_{dr}^2$ (weak in the two-photon regime). Their separation in our case is not in frequency but in the Hilbert subspaces they act upon.

In \autoref{fig:main_scheme}, we elaborate on how exactly the A-T effect occurs for feature III. \autoref{fig:main_scheme}~(a) demonstrates the unmodified level structure of the SAM. Orange (blue) levels correspond to first (second) qubit excited, the other being in the ground state. Orange ellipses denote the driving (detuned by $\Delta_1$ from $\omega_1$) of the first transmon. Note that it acts upon every pair of states $\ket{0j},\,\ket{1j},\ j=0..2$. The off-resonant two-photon drive on the second qubit is not shown here. Since from numerical simulation we know that level $\ket{02}$ is not necessary to observe the splitting, it is only shown with a dashed line. From here, we consider the first transmon to be a two-level qubit: $\ket{22}$ is not shown, $\hat H_{int}$ in RWA couples only $\ket{01},\,\ket{10}$ and $\ket{11},\,\ket{12}$.

The next step is to view the system in the frame rotating with the first transmon (\autoref{fig:main_scheme}~(b)). Just as in Appendix \ref{sec:3-level-at}, after rediagonalization it may be regarded as dressed by light, and the whole level system rearranges. Now, the first transmon splitting equals $\Omega_{1}^R = \sqrt{\Omega_1^2+\Delta_1^2}$, and its the new eigenstates (dressed states) are $\ket{a}$ and $\ket{b}$. The second transmon energies stay the same, and thus in overall we obtain just three doublets.

Mathematically, this procedure looks as follows. In the calculation below, we will use the following rotation operator (different from \eqref{eq:R}) to move to the rotating frame with \eqref{eq:rotation}:
\begin{equation}
R = \exp[-i\omega_d^1 t (b^{\dagger}b+c^{\dagger}c)]
\end{equation}  

Note that for brevity reasons we rotate both transmon subspaces simultaneously (in contrast to what is shown in \autoref{fig:main_scheme}). Moreover, we consider a general case when $\omega_d^1 - \omega_d^2 = \delta \neq 0 $ and $\Delta_{1,2} = \omega_d^1-\omega_{1,2}$ and $\omega_1 - \omega_2$ not necessary equals $\alpha_2/2$. The resulting Hamiltonian with RWA:
...
\begin{widetext}
	...
	\text{$H^R$} = \kbordermatrix{
		...
		\ket{00}& 0 & \text{$\Omega_1/2 $} & \text{$\Omega _2$} e^{i \text{$\delta$} t}/2 & 0 & 0 & 0 \\
		\ket{10} & \text{$\Omega_1/2 $} & \text{$-\Delta_1 $} & J & \text{$\Omega_2$} e^{i \text{$\delta$} t}/2 & 0 & 0 \\
		\ket{01} & \text{$\Omega_2$} e^{-i \text{$\delta$} t}/2 & J & -\text{$\Delta_2 $} &
		\text{$\Omega_1$/2} & \sqrt{2} \text{$\Omega_2$} e^{i \text{$\delta$} t}/2 & 0 \\
		\ket{11} & 0 & \text{$\Omega_2$} e^{-i \text{$\delta $} t}/2 & \text{$\Omega_1/2$} &
		\text{$-\Delta_2 -\Delta_1$} & \sqrt{2} J & \sqrt{2} \text{$\Omega_2$} e^{i \text{$\delta
				$} t}/2 \\
		\ket{02} & 0 & 0 & \sqrt{2} \text{$\Omega_2$} e^{-i \text{$\delta$} t}/2 & \sqrt{2} J &
		\text{$\alpha$}-2 \text{$\Delta_2$} & \text{$\Omega_1/2$} \\
		\ket{12} & 0 & 0 & 0 & \sqrt{2} \text{$\Omega_2$} e^{-i \text{$\delta $} t}/2 & \text{$\Omega_1/2
			$} & \text{$\alpha$}-2 \text{$\Delta_2$}- \text{$\Delta_1$} \\
	}
	\end{equation}
\end{widetext}
Here , $\delta = \omega_{dr}^1 - \omega_{dr}^2$.

Finally, in \autoref{fig:main_scheme}
